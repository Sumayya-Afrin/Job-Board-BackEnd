...
import { Controller, Post, Body, Param, Put, Delete, UseGuards, Req } from '@nestjs/common';
...
export class JobsController {
  ...
  async deleteJob(@Param('id') id: string) {
    ...
    @Post(':id/apply')
    @UseGuards(JwtAuthGuard) // Protect the route with authentication guard
    async applyForJob(@Param('id') jobId: string, @Body() applicationData, @Req() req: Request) {
